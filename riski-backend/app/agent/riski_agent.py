...
def _format_document_bullets(documents: list[Document]) -> str:
    """Create a markdown bullet list summarising retrieved documents."""
    bullets = []
    for doc in documents[:3]:
        metadata = doc.metadata or {}
        title = metadata.get("title", "Dokument")
        source = metadata.get("source", "mock://documents")
        snippet = doc.page_content or "Keine Zusammenfassung verfügbar."
        score = metadata.get("score")
        score_suffix = " (Relevanz %.2f)" % score if isinstance(score, (int, float)) else ""
        bullets.append("- **%s**%s — %s (%s)" % (title, score_suffix, snippet, source))
    if not bullets:
        bullets.append("- Keine Dokumente gefunden, bitte später erneut versuchen.")
    return "\n".join(bullets)
...
def _generate(state: RiskiAgentState) -> dict[str, list[BaseMessage]]:
    ...
    bullets = _format_document_bullets(documents)
    content = (
        "Ich habe folgende Hinweise zu '%s' zusammengetragen:\n\n" % question
        + "Gefundene Unterlagen:\n"
        + bullets
        + "\n\n"
        + "\n\n"
        + "Dies ist ein höchstriskanter MOCK Agent. Bitte möglichst Bald durch einen echten Assistenten ersetzten!"
    )
