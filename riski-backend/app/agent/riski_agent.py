...
DIRECT_ANSWER_TOOL_NAMES = {get_agent_capabilities.name, query_ris_database.name}
...
def _is_direct_tool_answer(messages: list[AnyMessage]) -> bool:
    """Return True if the last AIMessage was generated from a direct-answer tool."""
    # Walk backwards: skip the final AIMessage, then inspect the immediately
    # preceding message.  Direct-answer tools are routed back to the model and
    # must end after that final model answer instead of going through the
    # document guard.
    found_last_ai = False
    for msg in reversed(messages):
        if not found_last_ai:
            if isinstance(msg, AIMessage) and not msg.tool_calls:
                found_last_ai = True
            continue
        return isinstance(msg, ToolMessage) and msg.name in DIRECT_ANSWER_TOOL_NAMES
    return False
...
def _route_after_model(state: RiskiAgentState) -> str:
    ...
    - model just answered a direct-answer tool question → END
    ...
    if isinstance(last_message, AIMessage) and last_message.tool_calls:
        return NODE_TOOLS
    if _is_direct_tool_answer(state["messages"]):
        return END
...
def _route_after_tools(state: RiskiAgentState) -> str:
    ...
    - A direct-answer tool was called → back to model for final LLM answer.
    - Otherwise → guard as usual.
    """
    if state.has_error:
        return END
    last_message = state["messages"][-1] if state["messages"] else None
    if isinstance(last_message, ToolMessage) and last_message.name in DIRECT_ANSWER_TOOL_NAMES:
        return NODE_MODEL
    return NODE_GUARD
...
def build_guard_nodes(
    ...
    async def guard(state: RiskiAgentState, config: RunnableConfig) -> dict[str, Any]:
        ...
        # If only direct-answer tools were called, the answer should already be
        # generated by the model – guard should never be reached in this case,
        # but defend against it here to avoid a false no_tool_call/no-docs error.
        if has_any_tool_call and all(
            isinstance(m, ToolMessage) and m.name in DIRECT_ANSWER_TOOL_NAMES for m in messages if isinstance(m, ToolMessage)
        ):
            logger.warning("Guard reached after a direct-answer tool – this should not happen. Skipping.")
            return {}
...
def build_riski_graph(
    ...
    async def call_model(state: RiskiAgentState) -> RiskiAgentStateUpdate:
        ...
        # -- Direct-tool pass: model was routed back after a non-document tool --
        ...
        last_message = state["messages"][-1] if state["messages"] else None
        if isinstance(last_message, ToolMessage) and last_message.name in DIRECT_ANSWER_TOOL_NAMES:
            direct_messages: list[AnyMessage] = [system_message, *_sanitize_messages(state["messages"])]
            logger.debug(
                "Direct-tool pass message sequence: %s",
                [type(m).__name__ for m in direct_messages],
            )
            try:
                if force_llm_timeout:
                    raise APITimeoutError.__new__(APITimeoutError)
                response = await chat_model.ainvoke(direct_messages)
            except APITimeoutError:
                logger.warning("call_model: direct-tool-pass LLM call timed out.")
                return {
                    "messages": [],
                    "error_info": ErrorInfo(
                        error_type="timeout",
    ...
    async def run_tools(state: RiskiAgentState) -> RiskiAgentStateUpdate:
        ...
        Special case: if a direct-answer tool was called, only that tool message
        is returned and the model node creates the final answer from it.
        """
        ...
        # -- Short-circuit for direct-answer tools ---------------------------
        for msg in tool_messages:
            if isinstance(msg, ToolMessage) and msg.name in DIRECT_ANSWER_TOOL_NAMES:
