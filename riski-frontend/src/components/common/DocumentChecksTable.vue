...
<template>
    <section class="step-doc-checks" aria-labelledby="doc-checks-heading">
        <div class="doc-checks-header-row">
            ...
            <button v-if="hasReasoning" type="button" class="doc-checks-toggle" @click="showReasons = !showReasons"
                :aria-expanded="showReasons" aria-controls="doc-checks-content">
        ...
        <div id="doc-checks-content">
            <!-- Accepted Documents -->
            <div v-if="acceptedDocuments.length > 0" class="doc-section">
                <table class="doc-checks-table" aria-labelledby="accepted-docs-heading">
                    <caption class="sr-only">
                        Liste der {{ acceptedDocuments.length }} relevanten Dokumente
                    </caption>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        <tr v-for="(check, ci) in acceptedDocuments" :key="ci">
                            <td class="doc-check-name">
                                <a v-if="resolveDocUrl(check)" :href="resolveDocUrl(check)" target="_blank"
                            ...
                            <td v-if="showReasons" class="doc-check-reason">
                                {{ check.reason || "–" }}
                            </td>
                        </tr>
                    </tbody>
            ...
            <!-- Rejected Documents -->
            <div v-if="rejectedDocuments.length > 0" class="doc-section">
                <table class="doc-checks-table" aria-labelledby="rejected-docs-heading">
                    <caption class="sr-only">
                        Liste der {{ rejectedDocuments.length }} nicht relevanten Dokumente
                    </caption>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        <tr v-for="(check, ci) in rejectedDocuments" :key="ci">
                            <td class="doc-check-name">
                                <a v-if="resolveDocUrl(check)" :href="resolveDocUrl(check)" target="_blank"
                                    rel="noopener noreferrer" class="doc-check-link"
                                    :aria-label="`Dokument öffnen: ${check.name} (öffnet in neuem Fenster)`">
