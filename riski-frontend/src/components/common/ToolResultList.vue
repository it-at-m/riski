  ...
  <div class="tool-results">
    ...
    >
      ...
      <span class="summary-text">
        ...
        <template
          v-if="result.proposals.length > 0 && result.documents.length > 0"
          >,
        </template>
    ...
    >
      <div
        v-if="result.proposals.length > 0"
        class="result-group"
      >
        ...
        <ul class="result-list">
          ...
          >
            <a
              ...
              class="result-link"
              :title="proposal.name"
              >{{ proposal.identifier }} – {{ truncate(proposal.name) }}</a
            >
            <span v-else
              >{{ proposal.identifier }} – {{ truncate(proposal.name) }}</span
            >
          </li>
        </ul>
      </div>
